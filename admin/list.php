...
$('#tb_departments').bootstrapTable({
    ...
    columns: [{
        ...
        title: '账号/密码',
        formatter:function(value,row){
			return '<div>' + row.user + '<br/><span class="pass-hidden" data-pass="'+row.pass_decrypted+'">******</span><a href="#!" onclick="togglePass(this)" style="margin-left:5px;"><i class="mdi mdi-eye h6"></i></a></div>';
		}
    }, {
        field: 'sqluserpass',
        title: 'SQL账号/密码',
        formatter:function(value,row){
			return '<div>' + row.sqluser + '<br/><span class="pass-hidden" data-pass="'+row.sqlpass_decrypted+'">******</span><a href="#!" onclick="togglePass(this)" style="margin-left:5px;"><i class="mdi mdi-eye h6"></i></a></div>';
		}
    }, {
        field: 'webkj',
        title: '网页空间',
...
document.getElementById("zjpass").value = row.pass_decrypted;
document.getElementById("sqluser").value = row.sqluser;
document.getElementById("sqlpass").value = row.sqlpass_decrypted;
...
// 密码显示/隐藏切换函数
function togglePass(linkElement) {
    var spanElement = $(linkElement).prev('span.pass-hidden');
    var iconElement = $(linkElement).find('i');
    var realPass = spanElement.attr('data-pass');

    if (spanElement.text() === '******') {
        spanElement.text(realPass);
        iconElement.removeClass('mdi-eye').addClass('mdi-eye-off');
    } else {
        spanElement.text('******');
        iconElement.removeClass('mdi-eye-off').addClass('mdi-eye');
    }
}
