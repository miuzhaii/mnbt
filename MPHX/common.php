...
if(isset($dbconfig['sys_key']) && !empty($dbconfig['sys_key'])) {
    define('SYS_KEY', $dbconfig['sys_key']);
} else {
    // 向后兼容：如果配置文件中没有 sys_key，使用默认值（不安全）
    define('SYS_KEY', 'MNBT');
    // 建议运行 migrate_key.php 来生成新的随机密钥
}
...
if($conf) {
    $conf['pwd']=mn_decrypt($conf['pwd']);
    if(isset($conf['mailpassword'])) {
        $conf['mailpassword']=mn_decrypt($conf['mailpassword']);
    }
}
