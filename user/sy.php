...
msalert(4,'获取配置失败：数据解析错误', 3000);
console.error('解析错误:', date, e);
msloadingde();
}
}).fail(function(xhr, status, error) {
msalert(4,'获取配置失败：网络错误或服务器异常', 3000);
...
try {
    ...
    if(jsoe.code) {
        ...
        msalert(4,'刷新失败：' + (jsoe.msg || '未知错误'), 3000);
    }
} catch(e) {
    msalert(4,'刷新失败：服务器返回数据格式错误', 3000);
    console.error('解析错误:', date);
}
msloadingde();  // 关闭加载提示
}).fail(function(xhr, status, error) {
    msalert(4,'刷新失败：网络错误或服务器异常', 3000);
    msloadingde();  // 关闭加载提示
    console.error('请求失败:', error);
});
}
