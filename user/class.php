...
var $BT_KEY;
    ...
    public function Databasedownload($filename)  //下载数据库备份
    {
        $url = $this->BT_PANEL.'/files?action=GetFileBody';
        $p_data = $this->GetKeyData();        //取签名
        $p_data['path'] = '/www/backup/database/'.$filename; //备份文件的路径
        $result = $this->HttpPostCookie($url,$p_data);
        $data = json_decode($result,true);
        if($data['status']){
...
var $BT_KEY;
    ...
    public function setwjt($dat){            //设置伪静态
        // 首先验证伪静态规则
        $validation = $this->validateNginxRewrite($dat[1]);
        if (!$validation['valid']) {
            return ['status' => false, 'msg' => '伪静态规则验证失败：' . $validation['error']];
        }
     ...
     * 验证nginx伪静态规则
     */
    private function validateNginxRewrite($rules) {
        // 基本安全检查
        $dangerous_patterns = [
            '/eval\s*\(/i',
            '/exec\s*\(/i',
            ...
            '/`[^`]*`/',  // 反引号
            '/\$\{[^}]*\}/',  // 变量替换
            '/proxy_pass\s+http[s]?:\/\/[^;]+;/i',  // 代理到外部
            '/include\s+[^;]+;/i'  // 包含外部文件
        ];
        
        foreach ($dangerous_patterns as $pattern) {
            if (preg_match($pattern, $rules)) {
                return ['valid' => false, 'error' => '包含不安全的指令或代码'];
            }
        }
        
        // 检查语法错误
        $syntax_errors = $this->checkNginxSyntax($rules);
        if (!empty($syntax_errors)) {
            return ['valid' => false, 'error' => '语法错误：' . implode(', ', $syntax_errors)];
        }
     ...
     * 检查nginx语法
     */
    private function checkNginxSyntax($rules) {
        $errors = array();
        
        // 检查括号匹配
        if (substr_count($rules, '{') != substr_count($rules, '}')) {
            $errors[] = '大括号不匹配';
        }
        ...
        foreach ($lines as $line) {
            ...
            // 检查是否以分号结尾（除了某些特殊指令）
            if (!preg_match('/(location|if|rewrite|return|break)/i', $line) && 
                !preg_match('/\}$/', $line) && 
                !preg_match('/;$/', $line)) {
                $errors[] = '缺少分号';
                break;
        ...
        // 检查rewrite规则格式
        if (preg_match_all('/rewrite\s+(.+?)\s+(.+?)\s+(.+?);/i', $rules, $matches)) {
            foreach ($matches[0] as $rewrite) {
                if (!preg_match('/rewrite\s+.*\s+.*\s+(last|break|redirect|permanent|);/i', $rewrite)) {
                    $errors[] = 'rewrite规则缺少标记或分号';
                    break;
                }
            }
        }
