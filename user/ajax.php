...
<?php
if($egn=='login') {
	if(isset($_POST['user']) && isset($_POST['pass_hash'])) {
		$user=daddslashes($_POST['user']);
		$pass_hash=daddslashes($_POST['pass_hash']);
		...
		if ($conf['yzme']!='false' && $code != $_SESSION['authcode']) {
			...
			@header('Content-Type: text/html; charset=UTF-8');
			exit('{"code":"验证码错误！"}');
		} elseif($user==$wedsv['user']) {
			// 计算正确的哈希值 (密码 + 用户名)
			$correct_hash = hash('sha256', $wedsv['pass'] . $user);

			if($pass_hash === $correct_hash) {
				// 哈希验证成功
				unset($_SESSION['authcode']);
				$session=md5($user.$wedsv['pass'].$password_hash);
...
';
    ...
    // 确保流量数据为数字类型
    if(!isset($llskj['dq']) || !is_numeric($llskj['dq'])) $llskj['dq'] = 0;
    if(!isset($llskj['statistics']) || !is_numeric($llskj['statistics'])) $llskj['statistics'] = 0;
...
if($egn=='urllist') {
	...
	$sql="update `MN_zj` set `sqlpass` ='{$gpwd}', `pass` ='{$pass}' where `user`='{$user}'";
...
elseif($egn=='hqwj') {
	...
	if($s_data['status'] && $s_data['msg']!='') {
		...
		foreach($sfyr as $vfm) {
			...
			$g_size+=$e_size[6];
			if(isset($r_js['statistics']) && $arrMatches[0]==$r_js['statistics']) {
				$g_size=0;
			}
			$g_data=$arrMatches[0];
		}
		...
		// 如果无法获取日志，使用历史统计值或0
		$g_size = isset($ll_kjr['statistics']) && is_numeric($ll_kjr['statistics']) ? $ll_kjr['statistics'] : 0;
	}
	$r_jy=$ll_kjr;
	// 确保流量值是数字
	$ll_kjr['dq'] = is_numeric($g_size) ? $g_size : 0;  // 修复：直接赋值而不是累加，避免流量被重复计算
	$ll_kjr['statistics'] = is_numeric($g_size) ? $g_size : 0;
	$r_sy=json_encode($ll_kjr,256);
	$t_id=$yhc['id'];
	$sql="update `MN_zj` set `llmax` ='{$r_sy}' where `id`='{$t_id}'";

	// 更新数据库
	$DB->query($sql);  // 流量更新（所有产品都需要）
	if($yhc['hxc']!='1') {
		$DB->query($sql_w);  // 网页空间更新（非CDN产品）
		if($sql_s) $DB->query($sql_s);  // 数据库空间更新（非CDN产品）
	}

	// 检查是否超出限制并采取相应措施
	if($yhc['hxc']=='1') {
		// CDN产品只检查流量
		if($ll_kjr['dq'] > $ll_kjr['max']*1024*1024*1024) {
			$api->ztweb($yhc['btid'],$yhc['sqldz']);  // 停止网站
			$api->ftpxg($yhc['ftpid'],$yhc['user'],'0');  // 禁用FTP
		} else {
			$api->qdweb($yhc['btid'],$yhc['sqldz']);  // 启动网站
			$api->ftpxg($yhc['ftpid'],$yhc['user'],'1');  // 启用FTP
		}
	} else {
		// 非CDN产品检查所有资源
