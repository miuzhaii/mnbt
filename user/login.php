...
if($islogins==0){}else exit("<script language='javascript'>window.location.href='./index.php';</script>");
?>
...
// 使用 SHA256 哈希密码 (密码 + 用户名作为盐)
var passHash = CryptoJS.SHA256(passq + userq).toString();

let data = {};
data["gn"]="login";
data["user"]=userq;
data["pass_hash"]=passHash;  // 发送哈希值而不是明文密码
data["code"]=codeq;
